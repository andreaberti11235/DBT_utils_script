...
def draw_bbox_and_create_pil(npy_img, slice, x, y, width, height):
    mass_bbox = draw_box(image=npy_img[slice, :, :], x=x, y=y, width=width, height=height)
    mass_bbox = ((mass_bbox - np.amin(mass_bbox))/(np.amax(mass_bbox) - np.amin(mass_bbox)))*255
    mass_bbox = mass_bbox.astype(np.uint8)
    pil_mass = Image.fromarray(mass_bbox)
    return pil_mass
...
if __name__ == "__main__":
    ...
    for idx, row in tqdm(df_box.iterrows(), total=len(df_box.index)):
        ...
        pil_mass = draw_bbox_and_create_pil(npy_img, slice, x, y, max_dim, max_dim)
        out_file_name = img_name.split(sep='.')[0]
        out_file_name = f'{out_file_name}_mass{mass_count}.png'
        
        out_dir_path = os.path.join(dest_dir, 'whole_images', label)
